			...
			<fieldset>
			   <#list properties as property>
				<div class="form-group">
					<label class="col-md-2 control-label"> <spring:message code="${component.packageName}.${entity.className?uncap_first}.${property.propertyName}"/>
					</label>
					<div class="col-md-6">
					   ...
					   <#else>
					      <#if property.inputTypeText=="select">
					          <form:select path="${property.propertyName}" cssClass="form-control">
					            <form:option value="0" label="--Please Select" />
							    <form:options items="${r'${'}${property.propertyName}s${r'}'}" itemValue="key" itemLabel="value" />
						      </form:select>
						   <#elseif property.inputTypeText == "radio">
						       <form:radiobuttons path="${property.propertyName}" items="${r'${'}${property.propertyName}s${r'}'}" itemValue="key" itemLabel="value"   />
						   <#elseif property.inputTypeText== "checkbox">
						        <form:checkboxes path="${property.propertyName}" items="${r'${'}${property.propertyName}s${r'}'}" itemValue="key" itemLabel="value"/>
						    <#elseif property.inputTypeText=="textarea">
						        <form:textarea path="${property.propertyName}" cssClass="form-control" rows="5" cols="30" />
						   <#else>
