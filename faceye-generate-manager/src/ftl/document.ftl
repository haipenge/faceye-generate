...
<#list properties as property>
  <#if property.referenceEntity??>
    ...
    */
    @DBRef
    private  List<${property.referenceEntity.className}> ${property.referenceEntity.className?uncap_first}s=null;
    ...
    */
    @DBRef
    private  ${property.referenceEntity.className} ${property.referenceEntity.className?uncap_first}=null;
  ...
  <#else>
   ...
   <#if shortTypeClass == "Integer">
     <#assign propertyDefaultValue = "0">
   </#if>
   <#if shortTypeClass == "Long">
     <#assign propertyDefaultValue = "0L">
   </#if>
   <#if shortTypeClass == "Double">
     <#assign propertyDefaultValue = "0D">
   </#if>
   <#if shortTypeClass == "String">
     <#assign propertyDefaultValue = "\"\"">
   </#if>
   ...
   <#if property.inputTypeText =="radio" || property.inputTypeText =="select" || property.inputTypeText=="checkbox">
        public static Pairs<String, String> ${property.propertyName}s = new Pairs().add("1", "选项1").add("2", "选项2").add("3", "选项3");
        @Transient
        private  String ${property.propertyName}Text = ""; 
        public String get${property.propertyName?cap_first}Text() {
		  ${property.propertyName}Text = ${property.propertyName}s .getPair("" + ${property.propertyName}).getValue();
		  return ${property.propertyName}Text;
	     }
	    public void set${property.propertyName?cap_first}Text(String ${property.propertyName}Text) {
		  this.${property.propertyName}Text = ${property.propertyName}Text;
	     } 
   </#if>
    <#if property.isIndex>
     @Indexed
    </#if>
      private  ${shortTypeClass} ${property.propertyName} = ${propertyDefaultValue};
